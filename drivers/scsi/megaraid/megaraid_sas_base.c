        ...
        char *buf)
{
	...
	if (!instance->crash_dump_buf && !((instance->fw_crash_state == AVAILABLE) ||
       ...
       if (instance->fw_crash_buffer_offset > (instance->fw_crash_buffer_size * 1024 *1024)) {
        ...
        src_addr =  (unsigned long)instance->crash_buf[instance->fw_crash_buffer_offset/ (dmachunk -1) ] +
                        instance->fw_crash_buffer_offset % (dmachunk -1);
	...
	*/
	...
	if (ctrl_info && !megasas_get_ctrl_info(instance, ctrl_info)) {
		...
		if(instance->crash_dump_drv_support) { 
			printk("megasas: Crash dump is supported\n");
...
megasas_probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	/* Alloacte 1 MB DMA buffer for crash dump copy */	
	...
	if (cmd->frame->dcmd.opcode == MR_DRIVER_SET_APP_CRASHDUMP_MODE) {
		if (instance->crash_dump_drv_support && 
			...
			instance->crash_dump_app_support =
				cmd->frame->dcmd.mbox.w[0];
			printk("megasas: application crash dump mode set success\n");
			error = 0; /*SUCCESS*/
		} else {
			printk("megasas: application crash dump mode set failed\n");
			error = -1; /* FAILURE*/
		}
		megasas_return_cmd(instance, cmd);
		return error;
