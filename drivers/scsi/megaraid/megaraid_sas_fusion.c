			  ...
			  struct megasas_cmd_fusion *cmd)
{
	...
	if ((MR_TargetIdToLdGet(device_id, local_map_ptr) >= MAX_LOGICAL_DRIVES) || (!fusion->fast_path_io)) {
		...
		if (MR_BuildRaidContext(instance, &io_info, &io_request->RaidContext,
			  ...
			  struct scsi_cmnd *scmd, struct megasas_cmd_fusion *cmd)
{
	...
	if (scmd->device->channel < MEGASAS_MAX_PD_CHANNELS &&
		...
		os_timeout_value = scmd->request->timeout / HZ;
                ...
                else if (scmd->device->type == TYPE_DISK)
                        timeout_limit   = 255;
                else
                        timeout_limit   = 0xFFFF;

                if (os_timeout_value > timeout_limit)
                        io_request->RaidContext.timeoutValue =
                                timeout_limit;
                else
                        io_request->RaidContext.timeoutValue =
		...
		for (i = 0; i < MEGASAS_FUSION_MAX_RESET_TRIES; i++) {
			...
			for (j = 0 ; j < instance->max_fw_cmds; j++) {
				...
				if (cmd_fusion->sync_cmd_idx != (u32)ULONG_MAX) {
					cmd_mfi = instance->cmd_list[cmd_fusion->sync_cmd_idx];
					if (cmd_mfi->frame->dcmd.opcode == le32_to_cpu(MR_DCMD_LD_MAP_GET_INFO)) {
...
void  megasas_fusion_crash_dump_wq(struct work_struct *work)
{
	...
	index = (instance->drv_buf_index % MAX_CRASH_DUMP_SIZE);
	...
	/* 
	 * Driver has allocated <MAX_CRASH_DUMP_SIZE> buffers
	 * and FW has more crash dump data, then driver will 
	 * ignore the data (OR) Driver is done copying MAX_DUMP_SIZE
	 * buffers, still FW has not set MFI_STATE_CRASH_DUMP_DONE
	 * bit, so driver will set this bit
	 */
	if (index >= (instance->drv_buf_alloc)) {
		...
		memcpy(instance->crash_buf[index], instance->crash_dump_buf,
			CRASH_DMA_BUF_SIZE);
